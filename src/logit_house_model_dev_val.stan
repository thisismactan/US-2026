...
data {
  // Dev data
  int<lower=0> N_e;
  vector[N_e] logit_r2p;
  vector[N_e] logit_last_r2p;
  vector[N_e] midterm;
  ...
  // OOT data
  int<lower=0> N_oot;
  vector[N_oot] logit_r2p_oot;
  vector[N_oot] logit_last_r2p_oot;
...
model {
  vector[N_e] mu;
  a_midterm ~ normal(0, 1);
  b1_midterm ~ normal(0, 1);
  b1_redist ~ normal(-1, 1);
  b2_midterm ~ normal(0, 1);
  b2_redist ~ normal(-1, 1);
  b3_midterm ~ normal(0, 1);
  b3_redist ~ normal(1, 1);
  b3_contested_last ~ normal(1, 1);
  Rho ~ lkj_corr(1);
  tau ~ exponential(0.5);
  ...
  for (i in 1:N_e) {
    mu[i] = a_inc[incumbent_running[i]] + a_midterm * midterm[i] +
      (b1_inc[incumbent_running[i]] + b1_midterm * midterm[i] + b1_redist * redistricted[i]) * logit_last_r2p[i] * contested_last[i] +
  ...
  logit_r2p ~ normal(mu, sigma);
}

// Generated quantities
generated quantities {
  vector[N_e] mu_pred;
  vector[N_e] logit_r2p_pred;
  vector[N_oot] mu_oot;
  vector[N_oot] logit_r2p_pred_oot;
  // In-sample
  for (i in 1:N_e) {
    mu_pred[i] = a_inc[incumbent_running[i]] + a_midterm * midterm[i] + 
      ...
      (b2_inc[incumbent_running[i]] + b2_midterm * midterm[i] + b2_redist * redistricted[i]) * natl_r2p_change[i] +
      (b3_inc[incumbent_running[i]] + b3_midterm * midterm[i] + b3_redist * redistricted[i] + b3_contested_last * (1 - contested_last[i])) * last_pres_r2p[i];
    logit_r2p_pred[i] = normal_rng(mu_pred[i], sigma);
  }
  // OOT
  for (i in 1:N_oot) {
    mu_oot[i] = a_inc[incumbent_running_oot[i]] + a_midterm * midterm[i] + 
      (b1_inc[incumbent_running_oot[i]] + b1_midterm * midterm_oot[i] + b1_redist * redistricted_oot[i]) * logit_last_r2p_oot[i] * contested_last_oot[i] +
    ...
    logit_r2p_pred_oot[i] = normal_rng(mu_oot[i], sigma);
  }
}
