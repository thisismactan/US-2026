...
# State impact on forecast
senate_joint_state_majority_sims <- senate_state_posterior %>%
  select(sim_id, state, seat_name, r2p) %>%
  left_join(senate_seat_sims %>% select(sim_id, dem), by = "sim_id") %>%
  mutate(state_winner = ifelse(r2p > 0.5, "rep", "dem"),
         majority_winner = ifelse(dem >= 51, "dem", "rep")) %>%
  group_by(state, seat_name, state_winner, majority_winner) %>%
  summarise(n = n()) %>%
  spread(state_winner, n, fill = 0) %>%
  melt(measure.vars = c("dem", "rep"), variable.name = "state_winner", value.name = "n") %>%
  as_tibble() %>%
  arrange(state, seat_name, state_winner, majority_winner)

senate_state_predictive_accuracy_metrics <- senate_joint_state_majority_sims %>%
  left_join(senate_state_posterior_summary_stats %>% ungroup() %>% select(state, seat_name, r_prob),
            by = c("state", "seat_name")) %>%
  group_by(state, seat_name, r_prob) %>%
  mutate(dem_dem = (majority_winner == "dem") & (state_winner == "dem"),
         dem_rep = (majority_winner == "dem") & (state_winner == "rep"),
         rep_dem = (majority_winner == "rep") & (state_winner == "dem"),
         rep_rep = (majority_winner == "rep") & (state_winner == "rep")) %>%
...
senate_state_impact <- senate_state_predictive_accuracy_metrics %>%
  ...
  select(state, seat_name, r_prob, phi, dem_cond_increase, rep_cond_increase, total_cond_increase)

write_csv(senate_state_impact, "output/senate_state_impact.csv")